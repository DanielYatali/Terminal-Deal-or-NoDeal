...
int main(){
	...
	//the string entered is convert to a number to be used as number of cases
...
//The player can use these bonuses to their advantage to win the largest case value.
int bonus_event(Case cases, int num_cases,int shown_questions[], int count, Question questions[], int num_questions, int selector){
	string response;
	srand(time(NULL));
	string message;
	...
	if(response == questions[random_index].answer){
		...
		if(selector == 1){
			...
			//Bonus 2, changes the cases visibility so it can be seen
...
//Displays all cases to the board.
int display_all(Case cases, int num_cases){
	int n = 0;
	for(int i = 0; i < num_cases; i++){
...
void display_all_cases(Case cases, int n, int selector){
	...
	for(int i = 0; i < n; i++){
		...
		//Visiblity 0 and mask $$$$$$ : prints index and mask
		//Visiblity -1 : prints index and value
